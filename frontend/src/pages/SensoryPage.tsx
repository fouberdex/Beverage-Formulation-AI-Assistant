...
function AnalysisDashboard({ study, analytics, loading }: { study: SensoryStudy | null; analytics: SensoryStudyAnalytics | null; loading: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
      <ChartCard title="ANOVA screening" description="Exploratory one-way comparison across samples. Review assumptions and panel design before formal claims."><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="border-b bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-2">Attribute</th><th className="px-3 py-2">F</th><th className="px-3 py-2">df</th><th className="px-3 py-2">p-value</th><th className="px-3 py-2">η²</th><th className="px-3 py-2">Screen</th></tr></thead><tbody className="divide-y">{analytics.anova.map(item => <tr key={item.attribute_key}><th className="px-3 py-3 text-left font-medium">{item.attribute_label}</th>{item.result ? <><td className="px-3 py-3">{item.result.infinite_f ? '∞' : item.result.f_statistic}</td><td className="px-3 py-3">{item.result.df_between}, {item.result.df_within}</td><td className="px-3 py-3 font-mono">{item.result.p_value}</td><td className="px-3 py-3">{item.result.eta_squared}</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs font-semibold ${item.result.significant_at_0_05 ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>{item.result.significant_at_0_05 ? 'Difference signal' : 'No signal'}</span></td></> : <td colSpan={5} className="px-3 py-3 text-slate-400">Insufficient data</td>}</tr>)}</tbody></table></div></ChartCard>
