...
export default function LaboratoryResultsPage() {
  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="surface-card flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><label className="block flex-1 text-sm font-semibold text-slate-700">Exact formulation version analyzed<select value={formulationId} onChange={event => setFormulationId(event.target.value)} className="input mt-2"><option value="">Choose a formulation version</option>{formulations.map(formulation => <option key={formulation.id} value={formulation.id}>{formulation.name} · v{formulation.version} ({formulation.code}){formulation.project_id ? ' · R&D linked' : ''}</option>)}</select></label><div className="flex flex-wrap gap-2"><button type="button" onClick={template} className="secondary-button"><Download className="h-4 w-4"/> Template</button><button type="button" onClick={() => fileRef.current?.click()} disabled={!formulationId || importing} className="primary-button"><Upload className="h-4 w-4"/>{importing ? 'Importing…' : 'Import CSV / Excel'}</button><input ref={fileRef} type="file" accept=".csv,.xlsx,.xls" className="hidden" onChange={event => event.target.files?.[0] && void importFile(event.target.files[0])}/></div></section>
