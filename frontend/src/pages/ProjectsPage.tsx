...
export default function ProjectsPage() {
  ...
  const canManageProjects = hasPermission(profile?.role, 'manage_projects');
  ...
  return <div className="space-y-6">
    <header className="hero-panel">
      ...
      {canManageProjects && <button type="button" onClick={openCreate} className="primary-button"><Plus className="h-4 w-4"/> New project</button>}
    </header>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(24rem,.72fr)]">
      ...
      <aside className="surface-card h-fit xl:sticky xl:top-6">
        {!selected ? <div className="py-10 text-center"><Sparkles className="mx-auto h-9 w-9 text-sky-400"/><h2 className="mt-3 text-lg font-black">Select a project</h2><p className="mt-1 text-sm text-slate-500">Its gates, next actions and history will appear here.</p></div> : <>
          <div className="flex items-start justify-between gap-3"><div><p className="eyebrow">{selected.code}</p><h2 className="mt-1 text-xl font-black text-slate-950">{selected.name}</h2></div>{canManageProjects && <button type="button" onClick={openEdit} className="secondary-button">Edit</button>}</div>
          ...
          <div className="mt-5"><p className="eyebrow">Next controlled action</p>{canManageProjects && allowedTransitions.length ? <div className="mt-2 flex flex-wrap gap-2">{allowedTransitions.map(stage => <button key={stage} type="button" onClick={() => void transition(stage)} className="primary-button">Move to {stages.find(item => item.key === stage)?.label}<ArrowRight className="h-4 w-4"/></button>)}</div> : <p className="mt-2 text-sm text-slate-500">{canManageProjects ? 'No further transition is available.' : 'Your role has review-only access to project lifecycle transitions.'}</p>}</div>
