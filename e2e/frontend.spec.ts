...
async function mockApi(page: Page, options: { formulationStatus?: number; withProject?: boolean; projectBriefStatus?: 'draft' | 'validated' } = {}) {
  ...
  await page.route('**/api/v1/**', async (route: Route) => {
    ...
    if (path === '/supply-chain') return reply({ data: { suppliers: [], supplier_materials: [], material_specifications: [], packaging_components: [] } });
    ...
    const passport = { engine_version:'1.0.0',generated_at:'2026-09-15T10:00:00Z',project:{id:project.id,code:project.code,name:project.name,stage:project.stage,status:project.status,target_market:project.target_market,beverage_category:project.beverage_category},readiness:{score_percent:40,passed_gates:4,total_gates:10,status:'in_progress',gates:[{key:'validated_brief',label:'Validated R&D brief',passed:true},{key:'approved_formulation',label:'Approved exact formulation',passed:false}]},graph:{nodes:[{id:'project:project-1',entity_type:'project',entity_id:'project-1',label:'RD-2026-001 · Citrus launch',status:'active'}],edges:[],node_count:1,edge_count:0},summary:{formulations:1,laboratory_results:0,sensory_studies:0,experimental_plans:1,pilot_batches:1,stability_programs:0,product_specifications:0,documents:0,packaging_configurations:0,production_trials:0,qc_releases:0,quality_events:0,capa_actions:0},limitations:['This passport summarizes recorded workspace evidence; it is not a regulatory certificate or an automatic market-release authorization.'] };
