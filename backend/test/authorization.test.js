...
test('formulators cannot call administrator endpoints', () => {
  assert.equal(authorizeApiRequest({ method: 'PUT', path: '/api/v1/admin/users/user-1/role', role: USER_ROLES.ADMIN }).allowed, true);
  assert.equal(authorizeApiRequest({ method: 'PUT', path: '/api/v1/admin/users/user-1/role', role: USER_ROLES.FORMULATOR }).allowed, false);
});
