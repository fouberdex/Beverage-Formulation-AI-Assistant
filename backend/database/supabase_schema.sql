...
create table if not exists public.profiles (
  ...
  display_name text,
  role text not null default 'formulator' check (role in ('admin', 'rd_manager', 'formulator', 'lab', 'sensory', 'qa', 'regulatory', 'procurement', 'viewer')),
...
-- Commit only the rows changed by one authenticated API request. The backend
-- calls this with the service role after enforcing ownership and role checks;
-- all writes below run in this single PostgreSQL transaction.
